...
func (c *WSClient) reconnect() error {
	...
	for {
		jitterSeconds := time.Duration(r.Float64() * float64(time.Second)) // 1s == (1e9 ns)
		backoffDuration := jitterSeconds + ((1 << uint(attempt)) * time.Second)

		log.Info().Int("attempt", attempt+1).Dur("backoff_duration", backoffDuration).Msg("reconnecting")
...
func (c *WSClient) processBacklog() error {
	...
	case request := <-c.backlog:
		...
		if err := c.conn.WriteJSON(request); err != nil {
			log.Error().Err(err).Msg("failed to resend request")
			c.reconnectAfter <- err
			// requeue request
			c.backlog <- request
			return err
		}
		log.Info().Str("req", request.String()).Msg("resend a request")
	default:
	}
	return nil
}
	...
	for {
		...
		case originalError := <-c.reconnectAfter:
			...
			if err := c.reconnect(); err != nil {
				log.Error().Err(err).Str("original_err", originalError.Error()).Msg("failed to reconnect")
				c.Stop()
				return
			}
...
func (c *WSClient) readRoutine() {
	...
	for {
		...
		if err != nil {
			log.Error().Err(err).Str("data", string(data)).Msg("failed to parse response")
			continue
		}
		log.Info().Str("resp", string(response.Result)).Msg("got response")
